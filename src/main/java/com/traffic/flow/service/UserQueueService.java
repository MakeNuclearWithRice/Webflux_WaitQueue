...
public class UserQueueService {
    ...
    public Mono<Long> allowUser(final String queue, final Long count){
        return reactiveRedisTemplate.opsForZSet().popMin(USER_QUEUE_WAIT_KEY.formatted(queue), count)
                .flatMap(user -> reactiveRedisTemplate
                        .opsForZSet()
                        .add(USER_QUEUE_PROCEED_KEY.formatted(queue), user.getValue(), Instant.now().getEpochSecond()))
                .count();
    }
    ...
    public void scheduleAllowUser(){
        ...
        reactiveRedisTemplate.scan(ScanOptions.scanOptions()
                ...
                .doOnNext(tuple -> log.info("Tried %d and allowed %d users of % queue".formatted(maxAllowUserCount, tuple.getT2(), tuple.getT1())))
                .subscribe();
    }
}
